export const messages = {
  ...
  'Ready files expire about 1 hour after publication': ['Bereitgestellte Dateien verfallen etwa 1 Stunde nach der Veröffentlichung', 'Готовые файлы удаляются примерно через 1 час после публикации'],
  ...
  'Private to this browser session. Each final media file is limited to 5 GB, temporary session capacity is 12 GB, and ready results expire about 1 hour after publication. Collections may stop earlier when session storage fills.': ['Nur für diese Browsersitzung. Fertige Mediendateien sind auf 5 GB begrenzt, die Sitzung hat 12 GB temporären Speicher und bereitgestellte Ergebnisse verfallen etwa 1 Stunde nach der Veröffentlichung. Sammlungen können bei vollem Sitzungsspeicher früher enden.', 'Доступно только в этой сессии браузера. Итоговый медиафайл ограничен 5 ГБ, временное хранилище сессии — 12 ГБ, готовые результаты удаляются примерно через 1 час после публикации. Коллекция может остановиться раньше при заполнении хранилища.'],
  ...
  'Available through a private temporary link. The host processes the file, then publishes it for browser download. Waiting for cloud capacity does not reduce the 1-hour ready period.': ['Über einen privaten temporären Link verfügbar. Der Host verarbeitet die Datei und stellt sie anschließend zum Browser-Download bereit. Die Wartezeit auf Cloud-Kapazität verkürzt die einstündige Verfügbarkeit nicht.', 'Доступно по приватной временной ссылке. Сервер обрабатывает файл и затем публикует его для скачивания. Ожидание места в облаке не сокращает час доступности готового файла.'],
  'Processed by the host, then published through a private temporary download link.': ['Vom Host verarbeitet und anschließend über einen privaten temporären Download-Link bereitgestellt.', 'Обрабатывается сервером и публикуется по приватной временной ссылке.'],
  'Temporary processing + private cloud delivery': ['Temporäre Verarbeitung + private Cloud-Bereitstellung', 'Временная обработка + приватная облачная выдача'],
  ...
  'Waiting for delivery': ['Wartet auf Bereitstellung', 'Ожидание публикации'],
  'Publishing': ['Wird bereitgestellt', 'Публикация'],
  'Ready': ['Bereit', 'Готово'],
  'Expiring…': ['Wird entfernt…', 'Удаление…'],
  ...
  'Waiting for guarded cloud capacity': ['Wartet auf geschützte Cloud-Kapazität', 'Ожидание свободного места в облачном хранилище'],
  'Publishing securely for browser download…': ['Sichere Bereitstellung für den Browser-Download…', 'Безопасная публикация для загрузки в браузере…'],
  'Ready for download for about 1 hour': ['Etwa 1 Stunde zum Herunterladen bereit', 'Доступно для скачивания около 1 часа'],
  'Cloud publication was cancelled. The local result is available for retry.': ['Cloud-Bereitstellung abgebrochen. Das lokale Ergebnis kann erneut versucht werden.', 'Публикация в облако отменена. Локальный результат можно отправить повторно.'],
